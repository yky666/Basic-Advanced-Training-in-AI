...
TOLERANCE_LOOSE = 0.40    # Smoothness (Forward)
TOLERANCE_TIGHT = 0.10    # Precision (Reverse)
...
class PathManager:
    ...
    def run_nav_mode(self):
        ...
        while not rospy.is_shutdown():
            ...
            for idx, wp in enumerate(goals_list):
                ...
                # Check for shutdown inside the loop for fast exit
                if rospy.is_shutdown(): return
                
                # 1. Calculate Tolerance based on Distance to Next Point
                # -----------------------------------------------------
                # Check if this is the absolute last point of the file
                is_end_of_file = (idx == len(goals_list) - 1)
                
                if not is_end_of_file:
                    # Not the end? Look at the next point in the list
                    next_wp = goals_list[idx + 1]
                    dx = next_wp['x'] - wp['x']
                    dy = next_wp['y'] - wp['y']
                    segment_dist = math.sqrt(dx**2 + dy**2)
                else:
                    # End of file? Look at the FIRST point (Looping)
                    # This allows smooth transition from End -> Start of next lap
                    next_wp = goals_list[0]
                    dx = next_wp['x'] - wp['x']
                    dy = next_wp['y'] - wp['y']
                    segment_dist = math.sqrt(dx**2 + dy**2)

                # Determine Tolerance
                # If segment is long (>0.3m), use LOOSE tolerance (0.4m)
                # If segment is short (reverse/parking), use TIGHT tolerance (0.1m)
                if segment_dist > 0.3:
                ...
                # Log progress
                # rospy.loginfo(f"Lap {lap_count} - Point {display_idx}/{len(goals_list)}")

                # 2. Execute Navigation
                success = self.send_goal_with_retry(wp['x'], wp['y'], wp['z'], wp['w'], current_tolerance)
                
                if not success:
                    rospy.logwarn(f"Failed Waypoint {display_idx} (Lap {lap_count})")

            rospy.loginfo(f"Lap {lap_count} Completed.")
            # Optional: Short pause between laps
            # rospy.sleep(0.5) 
    ...
    def send_goal_with_retry(self, x, y, z, w, tolerance):
        goal = MoveBaseGoal()
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python3 record_and_nav_loop.py [record|nav]")
        sys.exit(1)
