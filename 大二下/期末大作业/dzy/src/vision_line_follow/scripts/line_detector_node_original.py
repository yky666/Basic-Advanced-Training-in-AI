...
class LineDetectorNode:
    def __init__(self):
        ...
        # ROI 参数：只看图像中下方地面区域
        self.roi_top_ratio = rospy.get_param("~roi_top_ratio", 0.35)
        ...
        # -----------------------------
        # 白色特点：饱和度低，亮度高
        # 但地面也偏白，所以后面只保留“红色附近的白色”
        self.white_lower = np.array(rospy.get_param("~white_lower", [0, 0, 145]), dtype=np.uint8)
        self.white_upper = np.array(rospy.get_param("~white_upper", [179, 95, 255]), dtype=np.uint8)

        # -----------------------------
        # 红色引导白色吸附参数
        # -----------------------------
        # 红色膨胀范围：用于吸附红色附近的白色胶带
        self.red_dilate_size = rospy.get_param("~red_dilate_size", 35)

        # 合成线区域后的形态学处理
        self.line_close_size = rospy.get_param("~line_close_size", 15)
    ...
    def image_callback(self, msg):
        ...
        # 3. 用红色区域引导白色胶带吸附
        # -----------------------------
        dilate_size = self.make_odd(self.red_dilate_size, 3)
        ...
        for cnt in contours:
            ...
            local_red = red_mask[y:y + bh, x:x + bw]
        ...
        if len(centers) >= self.min_points_for_fit:
            ...
            try:
                ...
                if self.lookahead_at_bottom:
                    # 优先使用 ROI 最底部作为目标点；
                    # 如果检测点没有覆盖到底部太远，则退回到最近检测点附近，避免严重外推
                    desired_y = roi_h - 1
        ...
        cv2.putText(
            debug,
            "mode: red-guided full line mask",
            (30, 80),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (255, 255, 255),
