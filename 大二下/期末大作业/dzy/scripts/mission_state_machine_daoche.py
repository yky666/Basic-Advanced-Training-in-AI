...
class MissionStateMachine:
    def __init__(self):
        ...
        self.recovery_back_timeout = float(rospy.get_param("~recovery_back_timeout", 8.0))
        self.recovery_back_wait = float(rospy.get_param("~recovery_back_wait", 0.3))
        self.after_b2_reverse_distance = float(rospy.get_param("~after_b2_reverse_distance", 2.0))
    ...
    def run_route_sequence(self, waypoints, text_after=None, semantic_waypoints=None, line_follow_after=None, skip_waypoints=None):
        semantic_waypoints = set(semantic_waypoints or [])
        skip_waypoints = set(skip_waypoints or [])
        for waypoint_name in waypoints:
            if waypoint_name in skip_waypoints:
                rospy.loginfo("Skipping waypoint: %s", waypoint_name)
                continue
            if waypoint_name not in self.waypoints:
            ...
            if waypoint_name in semantic_waypoints:
                ...
                if waypoint_name == self.b2_face_waypoint and self.after_b2_reverse_distance > 0.0:
                    self.publish_recovery_back_cmd(distance=self.after_b2_reverse_distance)
    ...
    def run(self):
        ...
        self.run_route_sequence(
            ...
            skip_waypoints=[self.b2_to_b1_mid_waypoint],
        )
